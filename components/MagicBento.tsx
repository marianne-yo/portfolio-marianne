...
const useBentoContext = () => {
  const ctx = useContext(BentoContext);
  if (!ctx) throw new Error('<MagicCard> must be used inside a <MagicBentoRoot>');
  return ctx;
};
...
const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    const diameter = spotlightRadius * 2; // ← ties blob size to the prop
...
export interface MagicBentoRootProps {
...
export const MagicBentoRoot: React.FC<MagicBentoRootProps> = ({
  children,
  className = '',
  enableStars = false,
  ...
  const ctxValue: BentoContextValue = {
    gridRef,
    glowColor,
...
export const MagicCard: React.FC<{
...
}> = ({ children, className = '', style, disableTilt = false, disableMagnetism = false }) => {
  const {
    glowColor,
    disableAnimations,
    particleCount,
    enableTilt: rootEnableTilt,       // ← rename on destructure
    enableMagnetism: rootEnableMagnetism, // ← rename on destructure
    clickEffect,
    tiltStrength,
    enableStars,
    enableBorderGlow
  ...
  const enableTilt = rootEnableTilt && !disableTilt;
  const enableMagnetism = rootEnableMagnetism && !disableMagnetism;
